...
export const metadata = {
  ...
  description:
    "تصفح كل كورسات اللغة العربية المتاحة مع الأستاذ أحمد بدوي على منصة المبدع. دروس فيديو HD لطلاب الثانوية العامة.",
...
export default async function CoursesPage({
  ...
  if (user) {
    ...
    if (profileData?.role === "student") {
      studentGrade = profileData.grade || null;
  ...
  let query = supabase
    .from("courses")
    .select(
      "id, title, description, price_cash, is_free, created_at, thumbnail_url, target_grade",
    )
    .eq("published", true)
    .is("deleted_at", null)
  ...
  const courses = enrolledCourseIds.length > 0
    ? (coursesRaw ?? []).filter((c) => !enrolledCourseIds.includes(c.id))
    : coursesRaw;
    ...
    <div className="bg-theme-primary min-h-screen">
      <div className="bg-gradient-to-r from-primary to-primary/70 py-10 md:py-16">
        <div className="container-custom text-center">
          <h1 className="text-4xl md:text-5xl lg:text-7xl font-payback font-bold text-white mb-4">
            كل الكورسات
          </h1>
          <p className="text-white/80 text-base md:text-lg">
            اختار كورس و يلا بينا يا فنان
          </p>
        </div>
      </div>
      ...
      <div className="container-custom py-12">
        <form method="GET" className="mb-10 max-w-xl mx-auto">
          <div className="relative">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-theme-secondary text-xl">
              🔍
            </span>
            <input
              type="text"
            ...
            {q && (
              <Link
                href="/courses"
                className="absolute right-4 top-1/2 -translate-y-1/2 text-theme-secondary hover:text-theme-primary font-bold transition-colors"
              >
                ✕
              </Link>
            )}
        ...
        {q && (
          <p className="text-theme-secondary mb-6 text-center">
            {courses?.length || 0} نتيجة لـ &ldquo;{q}&rdquo;
          </p>
        )}
        {error && (
          <div className="text-center py-20">
            <p className="text-red-400 font-semibold">
              حدثت مشكلة في تحميل الكورسات. حاول تاني.
            </p>
          </div>
        )}
        ...
        {!error && (!courses || courses.length === 0) && (
          <div className="text-center py-24 bg-theme-card rounded-2xl border-2 border-dashed border-[var(--border-color)]">
            ...
            <p className="text-theme-primary text-2xl font-bold mb-2">
              {q ? "مفيش كورسات" : "مفيش كورسات لحد دلوقتي"}
            </p>
            <p className="text-theme-secondary mb-6">
              {q
                ? `مفيش كورسات تطابق "‏${q}‏". جرب بحث تاني.`
                : "الكورسات هتظهر هنا بعد النشر."}
            </p>
            {q && (
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-[var(--border-color)]">
                    <span
                      className={`font-bold text-xl ${course.is_free ? "text-green-500" : "text-theme-primary"}`}
                    >
