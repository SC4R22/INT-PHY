...
export default function EditCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      if (error || !data) { setError('Course not found'); setLoading(false); return }
      setForm({
        title: data.title || '',
  ...
  const handleDelete = async () => {
    ...
    const { error: deleteError } = await supabase
      .from('courses').update({ deleted_at: new Date().toISOString() }).eq('id', id)
    if (deleteError) { setError(deleteError.message); setDeleting(false); return }
    router.push('/admin/courses')
  }

  if (loading) {
    return <div className="p-8 text-[#B3B3B3] animate-pulse">Loading...</div>
  }

  return (
    <div className="p-8 max-w-2xl">
      <div className="mb-8">
        <button onClick={() => router.push(`/admin/courses/${id}/content`)} className="text-[#B3B3B3] hover:text-[#EFEFEF] flex items-center gap-2 mb-4 transition-colors">
          ← Back to Content
        </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <button type="button" onClick={() => router.push(`/admin/courses/${id}/content`)}
