...
export function AdminSidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-64 bg-[var(--bg-card)] flex-col shrink-0 border-r-4" style={BORDER_GRADIENT_STYLE}>
      ...
      {/* Mobile top bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 flex items-center justify-between bg-[var(--bg-card)] px-4 h-14 border-b-4" style={BORDER_GRADIENT_STYLE}>
      ...
      {open && <div className="md:hidden fixed inset-0 z-30 bg-black/60" onClick={close} />}

      {/* Mobile drawer */}
      <aside className={`md:hidden fixed top-14 left-0 bottom-0 z-40 w-72 bg-[var(--bg-card)] flex flex-col transition-transform duration-300 border-r-4 ${open ? 'translate-x-0' : '-translate-x-full'}`}
