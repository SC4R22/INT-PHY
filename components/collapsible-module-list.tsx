            ...
            {isOpen && (
              <div className="border-t border-[var(--border-color)] bg-[var(--bg-primary)]">
                {sortedVideos.map((v, vIdx) => {
                  ...
                  if (isLocked) {
                    return (
                      <div key={v.id} className="flex items-center gap-3 px-5 py-3 opacity-40 cursor-not-allowed">
                        <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 bg-[var(--bg-card-alt)] text-xs">
                          🔒
                        </div>
                        <div className="min-w-0 flex-1 text-right">
                          <p className="text-xs font-semibold truncate text-theme-muted">{vIdx + 1}. {v.title}</p>
                          {v.duration && (
